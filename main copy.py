...
MONGO_URL = "mongodb+srv://jjen527:35OpRHzih8l1N14m@jjannik.aliwych.mongodb.net/"
...
class User:
    @classmethod
    async def create(cls, username, email, password, name, surname1, surname2, location, profile_image_path):
    ...
    @classmethod
    async def follow(cls, user_id, target_id):
    ...
    @classmethod
    async def delete_account(cls, user_id):
        obj_id = ObjectId(user_id)
        await users_collection.update_many({"followers": user_id}, {"$pull": {"followers": user_id}})
        await users_collection.update_many({"following": user_id}, {"$pull": {"following": user_id}})
        ...
        await posts_collection.update_many({"likes": user_id}, {"$pull": {"likes": user_id}})
        await posts_collection.delete_many({"userId": obj_id})
        await posts_collection.delete_many({"$and": [{"comments": {"$size": 0}}, {"likes": {"$size": 0}}]})
        await users_collection.delete_one({"_id": obj_id})

class Post:
    @classmethod
    async def create(cls, user_id, text, visibility, images_urls, videos_urls):
    ...
    @classmethod
    async def like(cls, post_id, user_id):
        await posts_collection.update_one(
            {"_id": ObjectId(post_id)},
            {"$addToSet": {"likes": user_id}}
        )
        post = await posts_collection.find_one({"_id": ObjectId(post_id)})
        return user_id in post.get("likes", []), len(post.get("likes", []))

    @classmethod
    async def dislike(cls, post_id, user_id):
        await posts_collection.update_one(
            {"_id": ObjectId(post_id)},
            {"$pull": {"likes": user_id}}
        )
    ...
    async def delete(cls, post_id, user_id):
        objpost = ObjectId(post_id)
        post = await posts_collection.find_one({"_id": objpost})
        if not post or post["userId"] != ObjectId(user_id):
            return False
...
class Comment:
    @classmethod
    async def create(cls, post_id, user_id, text):
        ...
        return result.inserted_id, creation_date

    @classmethod
    async def delete(cls, comment_id, user_id):
        objcomm = ObjectId(comment_id)
        comment = await comments_collection.find_one({"_id": objcomm})
        if not comment or comment["userId"] != ObjectId(user_id):
            return False
        await comments_collection.delete_one({"_id": objcomm})
        await posts_collection.update_many({"comments": objcomm}, {"$pull": {"comments": objcomm}})
        return True
...
async def register_user(
    ...
    await User.create(username, email, password, name, surname1, surname2, location, profile_image_path)
...
async def login_user(request: Request, username: str = Form(...), password: str = Form(...)):
    user, error = await User.authenticate(username, password)
...
async def delete_my_account(request: Request):
    ...
    await User.delete_account(user["id"])
...
async def delpost(request: Request, postid: str):
    ...
    success = await Post.delete(postid, user["id"])
    if not success:
...
async def delcom(request: Request, commentid: str):
    ...
    success = await Comment.delete(commentid, user["id"])
    if not success:
...
async def admin_delete_user(request: Request, userid: str):
    ...
    await User.delete_account(userid)
    print(f"Admin eliminó usuario {userid}")
...
async def create_post(
    ...
    await Post.create(user["id"], text, visibility, images_urls, videos_urls)
...
async def follow_user(request: Request, userid: str):
    ...
    await User.follow(user["id"], userid)
...
async def create_comment(request: Request, post_id: str = Form(...), text: str = Form(...)):
    ...
    inserted_id, creation_date = await Comment.create(post_id, user["id"], text)
...
async def like_post(request: Request, post_id: str):
    ...
    liked, likes_count = await Post.like(post_id, user["id"])
...
async def dislike_post(request: Request, post_id: str):
    ...
    liked, likes_count = await Post.dislike(post_id, user["id"])
    return JSONResponse({"liked": liked, "likes_count": likes_count})
    ...
    try:
        while True:
            ...
            if receiver_id and content:
                ...
                await manager.send_personal_message(send_msg, receiver_id)
                
                # Enviar de vuelta al remitente
                await manager.send_personal_message(send_msg, client_id)
